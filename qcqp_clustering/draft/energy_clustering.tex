...
applied to clustering, which makes connections to 
...
$\mu_2 = 5$, $\sigma_2 = 2$.}
\end{equation}
For these distributions the optimal classification accuracy 
obtained from Bayes error
...
These two simple experiments from Fig.\ref{fig:1d} illustrate
...
applied to \eqref{eq:maxQ}. This gives a local and approximate solution to 
...
The next result indicates that Hartigan's has the potential to
escape local optima of Lloyd's.
...
%Notice that this is a convention, 
%we could have chosen
%any other semimetric/kernel as the standard.
...
in higher dimensions and with more than two clusters.
...
that Algorithm~\ref{algo}  be superior than Algorithm~\ref{kmeans_algo}.
...
we also show the best possible classification 
accuracy obtained from a numerical estimation
...
For both Algorithm~\ref{algo} and Algorithm~\ref{kmeans_algo} 
we use the standard
...
\eqref{eq:rhoe}. The results are shown in Fig.~\ref{fig:consist}a.
...
\eqref{eq:20gauss}. 
(b) Data as in \eqref{eq:20loggauss}.
}
\end{figure}
...
concentric circles. (d) MNIST handwritten digits.
Clustering results are in Table~\ref{table:other}
and Table~\ref{table:mnist}.
}
\end{figure}

Now consider the following choices of semimetric and 
...
complex two dimensional datasets. For each class in Fig.\ref{fig:other}a
we sample $200$ points. In each class of 
Fig.\ref{fig:other}b and Fig.\ref{fig:other}c we sample $400$ points.
...
and Algorithm~\ref{kmeans_algo}, besides $k$-means and GMM, for the kernels 
\eqref{eq:rhoalpha},
\eqref{eq:rhoesigma} and
\eqref{eq:rhogsigma}. We sample the data $10$ times and show the average
...
\end{equation}
which is computed from the sampled data $\{ x_i \}_{i=1}^n$.
We consider subsets of the  classes $\{0,1,\dotsc,9 \}$, 
where we sample $100$ points 
for each class. We cluster the resulting dataset 
...
approaches to the case of Algorithm~\ref{algo}.
