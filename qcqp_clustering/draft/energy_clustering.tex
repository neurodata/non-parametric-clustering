...
% @jovo: any kernel method is defined on a HKHS. 
...
% related to it. In the case of energy this was done by [6], and it's based 
% on this work that we have the theory presented in this paper
...
% @jovo: commented out below and included a phrase in the end
%Moreover, $k$-means
...
% @jovo: commented above and included this phrase instead
...
% @jovo: tried to fix above, you're right in what I meant
The well-known
...
% @gui: not sure what you mean "only one possible optimization problem".  we
% can reformulate it many different ways? also, below, "this" is ambiguous.
% @jovo: I had in mind that minimizing W is equivalent to maximizing S, 
% but it's confusing at this point so just removed the phrase and adapted
...
% @jovo: ok, will do that after we finish everything.
...
% @jovo: ok, I will do that for the main symbols
...
% in some other paper I believe (will double check)
...
% @jovo: it's obviously a matrix
% I didn't want to write Y \in \mathbb{R}^{blabla} because it polutes the
% text too much and it's dimension is obviously the same as Z. Y was defined
% before also
...
% @jovo: in these cases there is no way, I have to. Look at the size
% of the expressions. I agree to not put reference to eq numbers
% for the main symbols, like S and W, but for large expressions we have to.
% I also try to avoid poluting the text with unnecessary math symbols, 
% it looks pretty bad
\end{proof}
...
% space" not in the feature space where data might be better separable. Maybe I
% need test this numerically
...
% @jovo: alright, will take care when re-generating the plots
...
% @jovo: I'm not sure what to say here, cuz all I'm doing is, given a data set
% I can compute \sigma as above. Then I use this in the kernel and perform
