...
def plot_gap(infile="experiments_data2/energy_synapse_gap.csv", 
    ...
    fig = plt.figure(figsize=(2*fig_width, 1*fig_height))
    # plot gaps
    ...
    ax.plot(xs, df["gap2"].values, color="r", linestyle="-", linewidth=1.5,
    ...
    ax.plot(xs, np.log(df["score"].values), '-og', linewidth=1.5, markersize=5)
    ax.set_xlim([1, len(xs)])
    ax.set_xlabel(xlabel)
...
def cluster_synapse(k, X, G):
    zh = energy_hartigan(k, X, G, return_labels=True)
    means = []
    for j in range(k):
        idx = np.where(zh==j)
        Y = X[idx]
...
if __name__ == '__main__':
    ...
    df = pd.read_csv("synapse_data/collman14v2.csv", dtype=float)
    X = df.values
    ...
    #elb4 = np.log(elb4)
    #elb4 = elb4[1:] - elb4[:-1]
    #plot_elbow_kernel([elb2, elb3, elb4], output='elbow.pdf')

    zh, means = cluster_synapse(8, X, G2)
    #plot_synapse_2d(X, zh, output='synapse_cluster_2d_k8.pdf')
    plot_synapse_pairs(X, zh, output='synapse_pairs_k8.pdf')
    
    zh, means = cluster_synapse(9, X, G4)
    #plot_synapse_2d(X, zh, output='synapse_cluster_2d_k9.pdf')
    plot_synapse_pairs(X, zh, output='synapse_pairs_k9.pdf')
