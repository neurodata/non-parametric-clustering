...
\title{Kernel k-Groups and Hartigan's Method}
...
through an equivalence between generalized energy distances and maximum
...
has such a long history in statistics and machine learning, making it
...
The predominant approach to clustering %, in Euclidean or kernel spaces,
is based on Lloyd's method.
However, there is an old heuristic
due to Hartigan \cite{Hartigan1975,Hartigan1979}, which apparently did
not receive the deserved attention, 
and goes as follows: for each data point, simply assign it to a cluster
...
which 
we do so in more generality. For instance, our approach is not limited
...
kernel k-groups 
is more accurate and robust, specially in high dimensions. 
...
even on Gaussian settings. We also illustrate the advantages of
kernel k-groups on a real dataset. 
...
%The standard metric from energy statistics is $\rho_1$ and 
...
%we use $\mathcal{E}^H$-clustering with the standard metric from energy
%statistics, i.e. \eqref{eq:rho_alpha} with \mbox{$\alpha=1$}. 
%We consider univariate
%normal and lognormal mixtures according to
%\begin{align}
...
%\label{eq:uni_params}
%\end{align}
%We increase the sample size $n$ and show the clustering accuracy 
%\eqref{eq:accuracy}
%versus $n$ in Fig.~\ref{fig:1D}. 
%In both cases the optimal accuracy 
%obtained from Bayes classification error
%is $ \approx 0.88$, indicated by the dashed line.
...
%with this lognormal data. Energy statistics on the other hand
%is model-free. We also show 
%a density estimation after clustering $1000$ points from these
%mixtures.
%Note that only $\mathcal{E}^{H}$-clustering
%was able to distinguish between the two classes, while 
%k-means and GMM put most of the
%points in a single cluster and points on the tail of the second component
%of \eqref{eq:uni_lognormal} in the other cluster.
%
%
%\begin{figure}
...
%\caption{
%\label{fig:1D}
%\emph{Top Left:} Clustering data from \eqref{eq:uni_normal}.
%\emph{Top Right:} Clustering data from \eqref{eq:uni_lognormal}.
%The dashed lines show the optimal Bayes accuracy, which in both cases 
%are $\approx 0.88$.
%\emph{Bottom Left:} 
%Density estimation of each component in the mixture \eqref{eq:uni_normal} 
%after clustering $1000$ sampled points.
%\emph{Bottom Right:} 
% The same but for lognormal data \eqref{eq:uni_lognormal}.
%}
%\end{figure}
...
The results are shown in Fig.~\ref{fig:gauss1}.
We see that 
...
\begin{figure}
\centering
\includegraphics[scale=0.5]{gauss1.pdf}
\caption{
\label{fig:gauss1}
High dimensional Gaussian mixture according to \eqref{eq:gauss1}.
We sample $200$ points total on each trial. The dashed line is
...
From Fig.~\ref{fig:gauss2} we see that 
...
\begin{figure}
\centering
\includegraphics[scale=0.5]{gauss2.pdf}
\caption{
\label{fig:gauss2}
...
which standard metric in energy statistics.
}
\end{figure}
...
the accuracy versus $n$ in Fig.~\ref{fig:gauss_n}. We compare
...
\begin{figure}
\centering
\includegraphics[scale=0.5]{gauss_n.pdf}
\caption{
\label{fig:gauss_n}
...
The results are shown in Fig.~\ref{fig:loggauss_n}.
We use exactly the same metrics as in the normal
mixture of Fig.~\ref{fig:gauss_n} to illustrate that the 
proposed method still performs accurately.

\begin{figure}
\centering
\includegraphics[scale=0.5]{loggauss_n.pdf}
\caption{
\label{fig:loggauss_n}
Same experiment as in Fig.~\ref{fig:gauss_n} but with a 
lognormal mixture with parameters \eqref{eq:20gauss}.
Again, kernel k-groups is more accurate than alternatives.
The figure suggests that neither of these methods are
...
more quickly than GMM. This is illustrated in Figure~\ref{fig:gauss_unbal}
...
\begin{figure}
\centering
\includegraphics[scale=0.5]{gauss_unbal.pdf}
\caption{
\label{fig:gauss_unbal}
...
\begin{tabular}{@{}l|ll@{}}
       Method      & Accuracy & aRand \\ \midrule[.5pt]
        kmeans       & 0.7131 & 0.6897 \\
         GMM         & 0.8770 & 0.8403 \\
 spectral clustering & 0.9536 & 0.9125 \\
    kernel k-means   & 0.7514 & 0.8510 \\
   kernel k-groups   & \textbf{0.9617} & \textbf{0.9356}
\end{tabular}
\end{table}
...
Algorithm~\ref{algo}, which we call kernel k-groups.
...
would is to extend the method to such 
situations, as well as a soft version.
