...
% @jovo: I corrected pretty much all your previous points, and also re-wrote 
% a lot of other stuff throughout the paper. 
% I deleted your previous comments which I believe were taken
% care of, since most of them were simple. The ones which were not, I will
% leave it, in the beggining of each section or in the middle.
%
...
% @jovo: I stoped saying non-parametric, however, Kolmogorov-Smirnov is also
...
% @jovo: I will do that for the main symbols, and refer to equations only
% when necessary. But sometimes it is unnavoidable, specially
% when referring to long 
% expressions or derivation steps. It is way more clear and precise to say, 
% replacing equation (1) into (2) we obtain ... than saying, 
% replacing A into B we obtain ... and the reader has to look around
% the whole paper to figure it out what  A, B are
...
% it is common to just put a parenthesis to avoid confusion, that's all

% @gui: this might be a better initialization than k-means++, which essentially initializes using a different kernel?
% @jovo: This was a long issue this last week as you know. 
% The spectral initialization is, often, very
% close to a local optimum of E-clustering algorithm, and kernel k-means
% as well. Now, since the set of local optima of Hartigan's is a subset of 
% those of Lloyd's, if you initialize "too well", you might get stuck on 
% a not so great local optimum of Lloyd's, which otherwise Hartigan's 
% would be able to escape. Numerically, in most cases Hartigan's does
% better when initializing with k-means++, or even at random.
...
%Since previous results should be already convincing enough that
%$\mathcal{E}^H$-clustering, based on Hartigan's method, is superior
%to $\mathcal{E}^L$-clustering, which is based on Lloyd's method, in the
%following 
