...
% @jovo: I did, removed some stuff that you didn't like before, now
% removing the non-parametric word. You think it's not good enough?
...
% @jovo: \usepackage{mathtools} \mathtoolsset{showonlyrefs}
...
% @jovo: I'll put a comment below, I simply removed the previous phrase,
% didn't say anything instead.
...
% @jovo: the previous paragraph saying that kernel k-means is a consequence
% of energy statistics WAS here, I removed and now don't say anything.
...
% @jovo: Removed the sentence. I have a very good reason to use E^S clustering
% instead of the spectral clustering from sklearn library. The reason is that
% E^S clustering is fundamentally different, being an exact solution to
% a relaxed problem, while 
% spectral clustering 
% from sklearn library is, apparently, Shi-Malik's method which is
% not directly related to the original problem.
% We have two options. (1) We keep these plots using E^S clustering proposed 
% before. (2) We remove E^S clustering 
% from the paper altogether, and if you want to compare with spectral 
% clustering at all we use sklearn library. However this will be totally 
% disconnected from the theory. 
...
% @jovo: what about Algorithm 1? All your points are clearly defined there.
% I don't think it's a good idea to initialize  E-H and E-L with E-S 
% because of Theorem 6. If you do this initialization you will not see much
% difference between E^L and E^H, missing the opportunity to illustrate a good
% selling point of this paper. What we can do, is to do a separate experiment
% comparing different initializations and see how the algorithms perform.
...
% @jovo: changed the phrase above. I could re-do all experiments again
% fliping a 1/2 coin for each distribution, but I don't think it's worthy since
% nothing will change and will just take extra time. So I just rephrased above.
\begin{figure}
\begin{minipage}{0.49\textwidth}
...
% @jovo: the difference is not increadibly large, but it is a litlle. I don't
% know why. And in Fig.3b this difference is very small, also don't know why
...
% @jovo: This is actually less suspicious since  I'm not even choosing the 
% numbers, just picking them at random. The point is to make the diagonal 
% entries different, so 2*ones does not help. Also, I don't want
% to keep trying many experiments everytime, it's super time consuming.
% This setting works to illustrate what we want ... a setting where k-means
% does poorly than GMM, and energy still beats GMM in high D.
...
% @jovo: I said explicitly "the fixed setting". 
% It is exactly as in eq. (59) with those numbers 
% fixed everytime, this is why I wrote the numbers explicitly. 
% I'm not generating new numbers everytime I sample.
...
% @jovo: see if above is ok, just said flexible instead
...
% @jovo: We can't claim we do worse on {0,1,2}, but also don't do better.
% I did the same experiment as Sapiro, and didn't fit in the table this is
% why I didn't include {0,...,9}, but I can do that.
% To treat the images better I would need to do some pre-processing, which
% can be time consuming, and you know how much I love manipulating images.
% I don't have any robust code for this, and anything I try to
% implement will take more than a week to do. I will check if I find something
% that works out of the box in python though.
