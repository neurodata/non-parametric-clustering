...
if __name__ == '__main__':
    ...
    #zh, means = cluster_synapse(6, X, G)
    zh, means = cluster_synapse(6, X, G2)
    print 
    print X.shape
    print
    print zh.shape
    print
    #f = open("cluster_membership.csv", "w")
    #for i in zh:
    #    f.write("%i\n"%i)
    #f.close()
    #g = sns.heatmap(means, 
    #                xticklabels=[r'$x_{%i}$'%(i+1) for i in range(11)],
    #                yticklabels=[r'$\mu_{%i}$'%(i+1) for i in range(6)]
    #)
    #sns.plt.yticks(rotation=0)
    #sns.plt.savefig("heat_means_k6_energy.pdf")
    #sns.plt.savefig("heat_means_k6_gauss.pdf")
    plot_synapse_2d(X, zh, output='synapse_cluster_2d_k6_gauss.pdf')
    #plot_synapse_pairs(X, zh, output='synapse_pairs_k6_gauss.pdf')
    #plot_synapse_2d(X, zh, output='synapse_cluster_2d_k6_energy.pdf')
    #plot_synapse_pairs(X, zh, output='synapse_pairs_k6_energy.pdf')
